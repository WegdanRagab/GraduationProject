...
def Report_page():
    ...
    def save_patient():
        ...
        if entry_name.get() == "" or entry_age.get() == "" or entry_first_data.get() == "" or entry_follow_up_data.get() == "" or entry_file_no.get() == "" or entry_sex.get() == "" or entry_refferig_doctor.get() == "" or entry_premedication.get() == "" or scr.get(
    ...
    label_head_result.place(x=5, y=40)
    entry_name = tk.Entry(textvariable=name, bd=1)
    entry_name.place(x=90, y=42, width=80)
    # text=entry_name.get()
    # name=StringVar()

    age = Label(text="   Age:  ", bg='#D1EAF0',
                fg='black', font=("Inter,bold"))
    age.place(x=160, y=35)
    entry_age = tk.Entry(textvariable=age, bd=1)
    entry_age.place(x=225, y=42, width=80)
    age = StringVar()

    data = Label(text="First Data:  ", bg='#D1EAF0',
                 fg='black', font=("Inter,bold"))
    data.place(x=300, y=35)
    entry_first_data = tk.Entry(textvariable=first_data, bd=1)
    entry_first_data.place(x=400, y=42)
    # first_data=StringVar()

    data_up = Label(text="Follow UP Data:  ", bg='#D1EAF0',
                    fg='black', font=("Inter,bold"))
    data_up.place(x=530, y=35)
    entry_follow_up_data = tk.Entry(textvariable=follow_up_data, bd=1)
    entry_follow_up_data.place(x=678, y=42)
    # follow_up_data=StringVar()

    file = Label(text="fileNO:  ", bg='#D1EAF0',
                 fg='black', font=("Inter,bold"))
    file.place(x=820, y=35)
    entry_file_no = tk.Entry(textvariable=file_no, bd=1)
    entry_file_no.place(x=885, y=42, width=120)
    # file_no=StringVar()

    sex = Label(text="Sex:  ", bg='#D1EAF0',
                fg='black', font=("Inter,bold"))
    sex.place(x=999, y=35)
    entry_sex = tk.Entry(textvariable=sex, bd=1)
    ...
    scr.place(x=130, y=150, width=750)
...
def video_record_page():
    ...
    def save_vid():
        ...
        file_path = filedialog.asksaveasfile(filetypes=file_types, defaultextension=file_types,initialdir=path2)
